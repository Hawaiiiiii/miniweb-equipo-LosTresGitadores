# Mini Web del Equipo - Los Tres Gitadores

## 📋 Descripción del Proyecto

Proyecto colaborativo desarrollado como parte de la asignatura **Sistemas de Control de Versiones** (2º DAW).

El objetivo es practicar el flujo de trabajo colaborativo con Git y GitHub, donde cada miembro del equipo añade su sección personal usando ramas, pull requests y revisión de código.

---

## 👥 Miembros del Equipo

### García Arenas, David Erik - 🎯 Coordinador
- Responsable de la creación y gestión del repositorio
- Coordinación del flujo de trabajo con ramas
- Revisión y aprobación de Pull Requests
- Resolución de conflictos y merge final

### Contreras López, Pablo
- Responsable del diseño visual y CSS avanzado
- *(Añadirá su sección en la siguiente fase)*

### Moreno Villegas, Iván
- Responsable de documentación y control de calidad
- *(Añadirá su sección en la siguiente fase)*

---

## 🛠️ Tecnologías Utilizadas

- **HTML5**: Estructura semántica
- **CSS3**: Diseño responsive con Grid y Flexbox
- **Git**: Control de versiones
- **GitHub**: Plataforma de colaboración

---

## 📂 Estructura del Proyecto

```
miniweb-equipo-LosTresGitadores/
├── index.html          # Página principal del equipo
├── style.css           # Estilos CSS personalizados
├── images/             # Carpeta de imágenes
│   └── erik.jpg        # Foto de David Erik
└── README.md           # Este archivo
```

---

## 🚀 Flujo de Trabajo Colaborativo

### Fase 1: ✅ David (Coordinador)
1. Crea el repositorio
2. Añade estructura base + su sección personal
3. Sube a `main`

<<<<<<< Updated upstream
### Fase 2: ⏳ Pablo
1. Clona el repositorio
2. Crea rama `seccion-pablo`
3. Añade su sección personal
4. Crea Pull Request
5. David revisa y aprueba

### Fase 3: ⏳ Iván
1. Actualiza su repositorio local
2. Crea rama `seccion-ivan`
3. Añade su sección personal
4. Crea Pull Request
5. David revisa y aprueba

### Fase 4: ⏳ David (Final)
1. Añade imagen grupal del equipo
2. Actualiza README con estadísticas finales
=======
### **Fase 2: ✅ Pablo**  
- Clona el repositorio  
- Crea rama `seccion-pablo`  
- Añade su sección personal  
- Crea *Pull Request*  
- David revisa y aprueba  

### **Fase 3: ✅ Iván**  
- Actualiza su repositorio local  
- Crea rama `seccion-ivan`  
- Añade su sección personal  
- Crea *Pull Request*  
- David revisa y aprueba  

### **Fase 4: ✅ David (Final)**  
- Añade **imagen grupal del equipo**  
- Actualiza el **README** con estadísticas finales  
>>>>>>> Stashed changes

---

## 📝 Estado Actual del Proyecto

<<<<<<< Updated upstream
- ✅ **Fase 1 completada**: Estructura base + sección de David
- ⏳ **Fase 2 pendiente**: Sección de Pablo
- ⏳ **Fase 3 pendiente**: Sección de Iván
- ⏳ **Fase 4 pendiente**: Imagen grupal final
=======
| Fase | Descripción | Estado |
|------|--------------|--------|
| 1 | Estructura base + sección de David | ✅ Completada |
| 2 | Sección de Pablo | ✅ Completada |
| 3 | Sección de Iván | ✅ Completada |
| 4 | Imagen grupal y actualización final | ✅ Completada |
>>>>>>> Stashed changes

---

## 📄 Licencia

Proyecto educativo para la asignatura de Sistemas de Control de Versiones.

© 2025 Equipo Los Tres Gitadores
